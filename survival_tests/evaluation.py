...
def evaluate(scenario: ASlibScenario, approach, metrics, amount_of_training_instances: int, fold: int, db_config, tune_hyperparameters: bool):
    ...
    logger.info("Evaluating \"" + approach.get_name() + "\" fold " + str(fold) + " training on " + str(
        amount_of_training_instances) + " scenario instances on scenario " + str(scenario[0].scenario))

    if tune_hyperparameters:
        optimizer = HyperParameterOptimizer()
        parametrization = optimizer.optimize(scenario[0], approach)
        approach.set_parameters(parametrization)

    train_status = db_config["EXPERIMENTS"]["train_status"]

    metric_results = evaluate_train_test_split((scenario[1], scenario[2]), approach, metrics, fold, amount_of_training_instances, train_status)

    for i, result in enumerate(metric_results):
        publish_results_to_database(db_config, scenario[0].scenario, fold, approach.get_name(), metrics[i].get_name(), result)
