...
def evaluate_train_test_split(scenario: ASlibScenario, approach, metrics, fold: int, amount_of_training_instances: int, train_status:str):
    ...
    # elif approach.get_name() in ['sbschedule']:
    #     approach.fit(train_scenario, test_scenario, fold, amount_of_training_instances)
    else:
    ...
    for instance_id in range(0, len(test_scenario.instances)):
        ...
        if contains_non_censored_value:
            ...
            for i, metric in enumerate(metrics):
                if approach.get_name() in ['schedule', 'sbschedule', 'nPARK_SBAlgorithm', 'nPARK_SBSchedule', 'nPARK_SBSchedule_to_SBAlgorithm', 'mean_optimization_time', 'unsolved_instances']: 
                    runtime = predicted_scores
                else:
                    runtime = metric.evaluate(y_test, predicted_scores, accumulated_feature_time, train_scenario.algorithm_cutoff_time)
